...
static u32 out_num = 0, cal_dis_out_num = 0;

EXP_ST u32 exec_tmout = EXEC_TIMEOUT; /* Configurable exec timeout (ms)   */
static u32 hang_tmout = EXEC_TIMEOUT; /* Timeout used for hang det (ms)   */
static u32 vmrss = 0;                 /* Memory usage                     */
...
EXP_ST u8  skip_deterministic,        /* Skip deterministic stages?       */
           ...
           read_testcase_time = 1;    /* Distinguish read or save_interest*/
...
EXP_ST u32 queued_paths,              /* Total number of queued testcases */
           ...
           max_dis_crash;             /* Maximum distance to crash paths  */
...
double last_exp3_ave;                 /* Average weights of last exp3     */
  
double sum_weights;                   /* The sum of weights for exp3      */

static u64 exp3_total_rounds = 0;         /* Total rounds for exp3            */
static u32 show_dis_crash;
static u32 queued_paths1 = 0, queued_paths2 = 0;
...
struct queue_entry {
  ...
  u8  cal_failed,                     /* Calibration failed?              */
      ...
      cause_crash;                    /* Causes crashes?                  */

  u32 bitmap_size,                    /* Number of bits set in bitmap     */
      exec_cksum,                     /* Checksum of the execution trace  */
      dis_crash;                      /* Distance to crash paths          */
  ...
  u8* crash_trace_mini;               /* Trace bytes, if crashed          */
  u8* trace_mini;                     /* Trace bytes, if kept             */
  u32 tc_ref;                         /* Trace bytes ref count            */
  double weights,                     /* Weights for exp3                 */
        score;
...
double min_score = MAP_SIZE * MAP_SIZE * 1.00, max_score = 0.0;
u32 min_bits = MAP_SIZE, max_bits = 0;
u32 min_dis = MAP_SIZE, max_dis = 0;
...
static void get_memory_usage() {
  FILE* file = fopen(STATUS_PATH, "r");
  if (!file) PFATAL("Unable to open '%s'", STATUS_PATH);
...
static void run_exp3(){
  double *probability_distribution = (double *)malloc(queued_paths * sizeof(double));
  if (!probability_distribution) PFATAL("run_exp3() failed");
  u32 t, i, chosen_arm, total_rounds;
  u32 min_bits = MAP_SIZE, max_bits = 0;
  double reward;
  double scaled_reward;
  struct queue_entry* q;
  double bestUpperBoundEstimate;
  double gamma;

  if (queued_paths < 100) total_rounds = 200;
  else if (queued_paths < 2000) total_rounds = 1000;
  else if (queued_paths < 10000) total_rounds = ( queued_paths / 1000 + 1) * 1000;
  else total_rounds = 5000;

  bestUpperBoundEstimate = 2 * total_rounds / 3.0;
  gamma = fmin(1, sqrt(queued_paths * log(queued_paths) / ((M_E - 1) * bestUpperBoundEstimate)));

  min_score = MAP_SIZE * MAP_SIZE * 1.0;
  max_score = 0.0;
  q = queue;
  while (q) {
    double score;
    if (q->dis_crash > 0) score = q->bitmap_size / q->dis_crash;
    else score = q->bitmap_size;
  ...
  for ( t = 0; t < total_rounds; t++)
  {
    ...
    while (q)
    {
      if ((sum_weights == 0.0) || (queued_paths == 0)){
        FATAL("sum_weights or queued_paths is 0");
        free(probability_distribution);
        return;
      }
      
      probability_distribution[i] = (1 - gamma) * (q->weights / sum_weights) + (gamma / queued_paths);
      i++;
      q = q->next;
    }
    ...
    chosen_arm = 0;
    while (q) {
        cumulative_probability += probability_distribution[chosen_arm];
        if (r < cumulative_probability) {
            break;
        }
        chosen_arm++;
        if (chosen_arm == queued_paths) {
          chosen_arm = queued_paths - 1;
          break;
        }
        q = q->next;
    }
    ...
    if ((max_score != min_score) && (max_score > min_score)) reward = (score - min_score) / (max_score - min_score);
    else reward = 1;

    scaled_reward = reward / probability_distribution[chosen_arm];
    sum_weights -= q->weights;
    q->weights *= exp((gamma / queued_paths) * scaled_reward);
    sum_weights += q->weights;
  }
  last_exp3_ave = sum_weights / queued_paths;

  free(probability_distribution);
}

/* Calculate shortest distance to crash paths */

static u32 cal_dis_to_crash(u8* mini_map) {
  u32 dis_min = max_dis_crash, i;
  struct queue_entry* q;
  q = queue;
  while (q)
  {
    if (q->cause_crash)
    {
      ...
      {
        u8 xor_result = mini_map[i] ^ q->crash_trace_mini[i];
        while (xor_result)
        {
          dis += xor_result & 1;
          xor_result >>= 1;
      ...
      if (dis >= 0 && dis < dis_min) dis_min = dis;
    }
    q = q->next;
  }
  return dis_min;
...
static void add_to_queue(u8* fname, u32 len, u8 passed_det) {
  ...
  q->n_fuzz       = 2;
  q->cause_crash  = 0;
  q->dis_crash    = max_dis_crash;
  if (read_testcase_time) {
    q->weights = 1.0;
    sum_weights += 1.0;
  }
  else {
    q->weights = last_exp3_ave;
    sum_weights += last_exp3_ave;
    // if (q->weights > max_weight) max_weight = q->weights;
  }
    ...
    if (trace_bits[i]) {
       ...
       if (top_rated[i]) {
         ...
         /* Faster-executing or smaller test cases are favored. */
		    ...
		    if (fav_factor < top_rated[i]->weights) continue;
...
static u8 calibrate_case(char** argv, struct queue_entry* q, u8* use_mem,
  ...
  if (!q->cause_crash) {
    cal_dis_out_num++;
    trace_mini_tmp = ck_alloc(MAP_SIZE >> 3);
    minimize_bits(trace_mini_tmp, trace_bits);
    q->dis_crash = cal_dis_to_crash(trace_mini_tmp);
...
keep_as_crash:
      ...
      out_num++;
      queue_top->cause_crash = 1;
      queue_top->dis_crash = 0;
      queue_top->crash_trace_mini = ck_alloc(MAP_SIZE >> 3);
      minimize_bits(queue_top->crash_trace_mini, trace_bits);
...
static void maybe_update_plot_file(double bitmap_cvg, double eps) {
  ...
  fprintf(plot_file,
          ...
          unique_hangs, max_depth, eps, vmrss); /* ignore errors */
...
static u32 calculate_score(struct queue_entry* q) {
  ...
  u32 avg_weight  = sum_weights / queued_paths;
  u32 perf_score = 100;
...
//   else if (q->bitmap_size * 1.5 < avg_bitmap_size) perf_score *= 0.75;
  ...
  int interval = MAP_SIZE / 5;

  if (q->dis_crash < interval) perf_score *= 5;
  else if (q->dis_crash < interval * 2) perf_score *= 4;
  else if (q->dis_crash < interval * 3) perf_score *= 3;
  else if (q->dis_crash < interval * 4) perf_score *= 2;
  else perf_score *= 1;

  show_dis_crash = q->dis_crash;

  if (q->weights * 0.3 > avg_weight) perf_score *= 3;
  else if (q->weights * 0.5 > avg_weight) perf_score *= 2;
  else if (q->weights * 0.75 > avg_weight) perf_score *= 1.5;
  else if (q->weights * 3 < avg_weight) perf_score *= 0.25;
  else if (q->weights * 2 < avg_weight) perf_score *= 0.5;
  else if (q->weights * 1.5 < avg_weight) perf_score *= 0.75;
...
EXP_ST void setup_dirs_fds(void) {
  ...
  fprintf(plot_file, "# unix_time, cycles_done, cur_path, paths_total, "
                     ...
                     "unique_hangs, max_depth, execs_per_sec, memory usage\n");
                     /* ignore errors */
